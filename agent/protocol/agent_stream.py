...
# Supported image extensions for inline vision
...
def _parse_image_refs(text: str) -> Tuple[str, List[str]]:
    """Extract [图片: /path] references from text, return clean text and image paths."""
...
def _build_image_blocks(image_paths: List[str]) -> List[Dict]:
    """Load images from local paths and build image_url content blocks."""
    blocks = []
    for path in image_paths:
        if not os.path.isfile(path):
            logger.warning(f"[ImageRef] File not found, skipping: {path}")
            continue
        ext = path.rsplit(".", 1)[-1].lower() if "." in path else ""
        if ext not in _IMAGE_EXTENSIONS:
            logger.warning(f"[ImageRef] Unsupported format .{ext}, skipping: {path}")
            continue
        try:
            ...
            if file_size > 1_048_576:
                read_path = _compress_image(path)
            with open(read_path, "rb") as f:
                b64 = base64.b64encode(f.read()).decode("ascii")
            if read_path != path and os.path.exists(read_path):
                os.remove(read_path)
            mime = _IMAGE_EXTENSIONS_MIME.get(ext, "image/jpeg")
            blocks.append({
                "type": "image_url",
                "image_url": {"url": f"data:{mime};base64,{b64}"},
            })
            logger.info(f"[ImageRef] Loaded image: {path} ({file_size // 1024}KB)")
        except Exception as e:
            logger.error(f"[ImageRef] Failed to load image {path}: {e}")
    return blocks


_IMAGE_EXTENSIONS_MIME = {
    "jpg": "image/jpeg", "jpeg": "image/jpeg",
    "png": "image/png", "gif": "image/gif", "webp": "image/webp",
}


def _compress_image(path: str) -> str:
    """Compress image to under 1MB, returns temp file path."""
    ...
    def _try_convert(max_dim: str, quality: str) -> bool:
        try:
            ...
            return False
    for max_dim, quality in [("1536", "85"), ("1536", "70")]:
        if _try_convert(max_dim, quality):
            if os.path.getsize(tmp.name) <= 1_048_576 or quality == "70":
                return tmp.name
...
class AgentStreamExecutor:
    ...
    def run_stream(self, user_message: str) -> str:
        ...
        # Add user message with inline image support
